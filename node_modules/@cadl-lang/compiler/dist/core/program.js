...
export async function createProgram(host, mainFile, options = {}) {
    const buildCbs = [];
    ...
    async function loadDirectory(dir, diagnosticTarget) {
        ...
        const mainFile = resolve(dir, typeof (pkg === null || pkg === void 0 ? void 0 : pkg.cadlMain) === "string" ? pkg.cadlMain : "main.cadl");
    ...
    async function loadJsFile(path, diagnosticTarget) {
        if (program.jsSourceFiles.has(path))
            return;
            ...
            if (path.startsWith("./") || path.startsWith("../")) {
                ...
                try {
                    ...
                    if (e.code === "MODULE_NOT_FOUND") {
                        program.reportDiagnostic(createDiagnostic({ code: "library-not-found", format: { path }, target: stmt }));
                        continue;
                    }
                    else {
                        throw e;
     ...
     */
    function resolveModuleSpecifier(specifier, basedir, useCadlMain = true) {
    ...
    async function loadMain(mainFile, options) {
    ...
    // compiler.
    async function checkForCompilerVersionMismatch(mainPath, mainPathIsDirectory) {
...
export async function compile(mainFile, host, options) {
