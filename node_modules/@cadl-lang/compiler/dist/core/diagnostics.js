    ...
    function createDiagnostic(diagnostic) {
        ...
        if (!diagnosticDef) {
            const codeStr = Object.keys(diagnostics)
                .map((x) => ` - ${x}`)
                .join("\n");
            throw new Error(`Unexpected diagnostic code '${diagnostic.code}'. ${errorMessage}. Defined codes:\n${codeStr}`);
        }
        ...
        if (!message) {
            const codeStr = Object.keys(diagnosticDef.messages)
                .map((x) => ` - ${x}`)
                .join("\n");
            throw new Error(`Unexpected message id '${diagnostic.messageId}'. ${errorMessage} for code '${diagnostic.code}'. Defined codes:\n${codeStr}`);
        }
...
export function computeTargetLocation(target) {
    let location;
    let locationError;
    ...
    if (locationError) {
        const diagnosticError = new Error("Error(s) occurred trying to resolve target: " + (target === null || target === void 0 ? void 0 : target.toString()));
...
export function dumpError(error, logger) {
    var _a;
    if (error instanceof AggregateError) {
        ...
        logger.log({ level: "error", message: (_a = error.stack) !== null && _a !== void 0 ? _a : "" });
    }
}
/**
